(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.sign = {}));
    ...
    ***************************************************************************** */
     ...
     * */
    var createSign = function (license, params, debugging) {
        if (params === void 0) { params = {}; }
        if (debugging === void 0) { debugging = false; }
        var keys = Object.keys(params).sort(function (a, b) {
            return a > b ? 1 : -1;
        });
        var KEYS = keys.reduce(function (cur, nxt) { return "".concat(cur).concat(nxt).concat(params[nxt]); }, '');
        var SALT = license.SALT;
        var SIGN = "".concat(KEYS).concat(SALT);
        if (debugging) {
            console.group('<<< sign >>>');
     ...
     * */
    var signFormat = function (license, data, options, prop) {
        if (options === void 0) { options = configDefault; }
        if (prop === void 0) { prop = propDefault; }
        if (!license) {
            return data;
        }
        var platform = license.PLATFORM, version = license.VERSION;
        if (!platform || !version) {
            return data;
        }
        var _a = options.token, token = _a === void 0 ? false : _a, _b = options.payload, payload = _b === void 0 ? false : _b, _c = options.offset, offset = _c === void 0 ? 0 : _c;
        var fieldToken = prop.token;
        var nonce = createRandomNumber(4);
        var timestamp = Math.floor(new Date().getTime() / 1000);
        if (typeof offset !== 'undefined') {
            timestamp += offset;
        }
        var params = { platform: platform, timestamp: timestamp, nonce: nonce, version: version };
        if (payload) {
        ...
        var sign = createSign(license, params, options.debug);
        return __assign(__assign(__assign({}, data), params), { sign: sign });
    };

    exports.createSign = createSign;
    exports.signFormat = signFormat;
